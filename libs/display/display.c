...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  uint16_t index = 0;
  char ver=c;
  index = (c - ' ') * 8; // Para letras maiúsculas
...
void draw_bpm(ssd1306_t *ssd, const char *str, int bpm, uint8_t x, uint8_t y) 
{
  char buffer[32];
  snprintf(buffer, sizeof(buffer), str, bpm);
  ssd1306_draw_string(ssd, buffer, x, y);
}

void draw_time(ssd1306_t *ssd, const char *str, int f, int s, uint8_t x, uint8_t y) 
{
  char buffer[32];
  snprintf(buffer, sizeof(buffer), str, f, s);
  ssd1306_draw_string(ssd, buffer, x, y);
}